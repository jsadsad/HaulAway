...
class JobShow extends React.PureComponent {
  ...
  closeJobButton() {
    ...
    if ((job.jobPoster._id === this.props.currentUserId) && (!job.isAvailable) &&(!job.isClosed)) {
      return (
  ...
  reviewJobButtons() {
    ...
    if ((job.jobPoster._id === this.props.currentUserId) && (!job.isReviewed) && (job.isClosed))
  ...
  render() {
    ...
    let haulRequester = ''
    if ( job.jobPoster ) {
      haulRequester = <div className="job-poster-name">Haul requested by: <Link to={`/users/${job.jobPoster}`}>{ job.jobPoster.firstName }</Link></div>
    }
