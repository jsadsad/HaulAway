...
class UserShow extends React.Component {
  ...
  componentDidMount() {
    this.props.fetchUsers().then(this.props.fetchUser(this.props.userId))
        ...
        if (job._id === review.jobId && this.props.user._id !== review.author) {
          ...
          return (
            <div key={index} className="review-info-container">
              <div
                className="review-info-index"
                onClick={() =>
                  this.props.history.push(
                    `/users/${this.props.userId}/reviews/${review._id}`
                  )
                }
              >
    ...
    if (totalReviewNum === 0) {
      return (
        <p className="no-review-message">
          Currently, there is no review for this user
        </p>
      )
    } else {
